"""`RouterNode(...)` beside `GraphNode(kind="router")`, and `isinstance` meaning it.

The point of these classes is dispatch and readability, so the thing worth
pinning is not that `RouterNode` exists — it is that **`isinstance` does not depend
...
def test_the_class_and_the_kind_argument_build_the_same_thing():
    a = Graph(name="t", outputs=["x"], nodes=[
        RouterNode(id="r", routes={"a": "x"}), BaseNode(id="x", depends_on=["r"])])
    b = Graph(name="t", outputs=["x"], nodes=[
        GraphNode(id="r", kind="router", routes={"a": "x"}),
        GraphNode(id="x", kind="base", depends_on=["r"])])

    assert type(a.nodes[0]) is type(b.nodes[0]) is RouterNode
...
def test_a_graph_loaded_from_yaml_gets_the_classes_too():
    ...
    assert isinstance(g.nodes[0], MapNode)
    assert isinstance(g.nodes[0].body[0], BaseNode), "bodies are upgraded too"
...
def test_the_wire_format_is_unchanged():
    ...
    dumped = RouterNode(id="r", routes={"a": "x"}).model_dump(exclude_none=True)
...
@pytest.mark.parametrize(
    ("cls", "kind"),
    [(BaseNode, "base"), (ReactNode, "react"), (RouterNode, "router"), (LoopNode, "loop"),
     (MapNode, "map"), (SubgraphNode, "subgraph"), (InputNode, "input"), (OutputNode, "output")],
)
...
def test_container_answers_has_a_body_in_one_question():
    ...
    assert isinstance(LoopNode(id="n"), ContainerNode)
    assert isinstance(MapNode(id="n"), ContainerNode)
    assert isinstance(SubgraphNode(id="n"), ContainerNode)
    assert not isinstance(BaseNode(id="n"), ContainerNode)
    assert not isinstance(RouterNode(id="n"), ContainerNode)
