...
FN = "tests.engine.test_graph_control_flow"
...
def test_routes_router_classifies_on_instructions():
    """A router written the new way must actually be told what to do.

    `instructions` replaced purpose/goal/expected_result, and the studio's
    instruction box writes it *and clears the older three*. The router built its
    classification prompt by hand from `purpose or goal`, so a router edited in
    the studio silently fell back to "Route for node route" — it still answered,
    still picked a branch, and had been told nothing. Nothing failed; it just
    classified worse.
    """
    spec = _routes_graph()
    spec["nodes"][0].pop("goal")
    spec["nodes"][0]["instructions"] = "Route this support ticket by urgency: {ticket}"
    graph = load_graph_from_dict(spec)
    ...
    assert "Route this support ticket by urgency" in prompt
    assert "the site is down" in prompt, "instructions must interpolate like goal did"
    assert "Route for node route" not in prompt


def test_router_classifies_on_its_own_provider():
    """"Route with the cheap model, write with the expensive one" is the reason
    per-node providers exist, and the router — the one kind whose whole job is a
    cheap decision — ignored both fields and used the run's default."""
    from neurosurfer.llm.resolver import ProviderResolver

    spec = _routes_graph()
    spec["nodes"][0]["provider"] = "cheap"
    graph = load_graph_from_dict(spec)

    run_default = ScriptedProvider(turns=[("urgent", [])])
    cheap = ScriptedProvider(turns=[("billing", [])])
    resolver = ProviderResolver(run_default, factory=lambda *_a, **_k: cheap,
                                profiles={"cheap": {}})

    res = GraphExecutor(graph, provider=run_default, provider_resolver=resolver,
                        log_traces=False).run({"ticket": "charged twice"})
    assert res.nodes["route"].raw_output == "finance"  # the cheap model answered
    assert cheap.calls == 1
    assert run_default.calls == 0


def test_routes_router_still_reads_the_legacy_fields():
    """Graphs on disk say it the old way, and have to keep working."""
    graph = load_graph_from_dict(_routes_graph())  # uses `goal`
    provider = ScriptedProvider(turns=[("urgent", [])])
    GraphExecutor(graph, provider=provider, log_traces=False).run({"ticket": "down"})
    assert "Route this support ticket by urgency" in provider.prompts[0]
