...
from neurosurfer.registry.core.system.install_package import (
...
from neurosurfer.registry.core.system.set_python_env import SetPythonEnvArgs, SetPythonEnvTool
...
class TestResolveInterpreter:
    ...
    def test_describe_never_provisions(self, tmp_path, monkeypatch):
        ...
        with patch(
            "neurosurfer.registry.core.system.python_exec.interpreter.ensure_managed_venv"
        ) as ensure:
...
class TestResolveEnvSpec:
    ...
    def test_conda_spec_found(self):
        ...
        with patch(
            "neurosurfer.registry.core.system.python_exec.interpreter.subprocess.run",
...
class TestInstallPythonPackage:
    ...
    async def test_skips_already_importable(self, tmp_path):
        ...
        with (
            patch(
                "neurosurfer.registry.core.system.install_package.resolve_interpreter",
                return_value="/usr/bin/python3",
            ),
            patch(
                "neurosurfer.registry.core.system.install_package._is_importable",
...
class TestSetPythonEnv:
    ...
    async def test_pins_interpreter_and_durable_state(self, tmp_path):
        ...
        with (
            patch(
                "neurosurfer.registry.core.system.set_python_env.resolve_env_spec",
                return_value="/some/python",
            ),
            patch(
                "neurosurfer.registry.core.system.set_python_env.installed_packages",
...
class TestEnvironmentSection:
    ...
    def test_renders_ready_interpreter_with_packages(self, tmp_path):
        with (
            patch(
                "neurosurfer.registry.core.system.python_exec.interpreter.describe_interpreter",
                return_value=("/fake/python", True),
            ),
            patch(
                "neurosurfer.registry.core.system.python_exec.managed_env.installed_packages",
