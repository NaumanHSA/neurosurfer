...
from neurosurfer.registry.core.system.python_exec import PythonExecTool
from neurosurfer.registry.core.system.python_exec.env import _SENSITIVE, build_sandbox_env
from neurosurfer.registry.core.system.python_exec.tool import PythonExecArgs
from neurosurfer.tools.base import ToolContext
from tests.fakes import ScriptedIO

# Marked `slow`: see the marker's note in pyproject.toml.
# Every case runs real code in a sandboxed interpreter subprocess, which is
# the point of the tool and costs ~9s a time.
...
class TestKeepSandbox:
    ...
    async def test_sandbox_cleaned_by_default(self):
        ...
        with patch("neurosurfer.registry.core.system.python_exec.sandbox.tempfile.mkdtemp", side_effect=recording_mkdtemp):
            tool = PythonExecTool()
