...
def test_a_tool_using_os_system_validates_on_its_contract_alone(tmp_path):
    """There used to be a substring scan raising "uses 'os.system'". It was
    removed: it matched inside comments and strings, fired on tools whose whole
    purpose was the flagged call, and blocked nothing — so it read as a security
    control while being none, and taught the eye to skip warnings. Validation
    judges the contract; judging the code is what reading it is for."""
    author = _author(_FakeProvider([]), tmp_path)

    result = author.validate_draft(
        ToolDraft("count_lines", _RAW_RISKY, ToolGapSpec("count_lines", "x"))
    )
