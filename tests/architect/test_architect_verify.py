...
FN = "tests.architect.test_architect_verify"
...
async def test_tool_failure_blocks_register_and_edit_stales(tmp_path, kb):
    ...
    # A real verification exists (it failed); changing the design stales it, because
    # the stored fingerprint no longer matches the graph.
...
async def test_verification_is_required_by_default(tmp_path, kb):
    """V3 Phase 5c. `encouraged` is prompt-only, and a weak model skips it: both
    bug-report transcripts went validate → ok → register with no test_workflow."""
    from neurosurfer.architect import ArchitectAgent

    # The session default, and the agent default that feeds it.
    assert BuildSession(
        intent="x", staging_root=tmp_path / "s",
        registry=WorkflowRegistry(workflows_dir=tmp_path / "r"),
        knowledge=kb,
    ).verification_mode == "required"
    assert ArchitectAgent(ScriptedProvider(turns=[]))._verify == "required"

    # A session left at the default refuses to register an untested graph.
    session = BuildSession(
        intent="double a number", staging_root=tmp_path / "staging2",
        registry=WorkflowRegistry(workflows_dir=tmp_path / "registry2"),
        knowledge=kb,
    )
    session.name = "doubler"
    session.inputs = [{"name": "x", "type": "integer", "required": True}]
    session.nodes = [{"id": "d", "kind": "function", "callable": f"{FN}._double"}]
    session.outputs = ["d"]
    ok, msg = session.register()
    assert not ok
    assert "requires a passing" in msg and "test_workflow" in msg


async def test_lowering_the_bar_is_an_explicit_caller_decision(tmp_path, kb):
    """The escape from `required` is the caller choosing it, never the model."""
    session = _session(tmp_path, kb, mode="encouraged")
    ok, _ = session.register()
    assert ok  # same graph, same absence of a verification — allowed because asked for
...
async def test_agent_closed_loop_required(tmp_path, kb):
    ...
    agent = ArchitectAgent(
        ...
        knowledge=kb,
        verify="required",
        plan=False,   # scripted builder turns; the planner has its own tests
    )
