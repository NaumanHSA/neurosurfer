...
class WebSearchTool(BaseTool):
    ...
    spec = ToolSpec(
        ...
        inputs=[
            ToolParam(name="query", type="string", description="The web search query.", required=True),
            ToolParam(name="hl", type="string", description="Interface language (e.g. 'en'). Defaults to 'en'.", required=False),
        ],
        returns=ToolReturn(
            type="object",
            description="JSON object with `query`, `summary`, and `results` (each result has `title`, `url`, `snippet`, `score`).",
        ),
    )
    ...
    def __init__(
        ...
        *,
        ...
        gl:
            Optional default `gl` string for geo-targeted searches.
        include_raw:
            If True, include the raw SerpAPI payload in `extras['raw']` for debugging or advanced use.
        """
        super().__init__()
