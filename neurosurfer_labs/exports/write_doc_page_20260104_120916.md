# Node `write_doc_page` output

- Mode: `text`
- Started at: `2026-01-04T12:09:16.130117`
- Duration: `20623` ms
- Error: `None`
...
## Overview
The `GraphExecutor` module is the core component for executing graph-based workflows in the Neurosurfer framework. It integrates with various agents and tools to manage the execution of graph structures, validate configurations, and collect results.
...
- **Responsibilities:**
  ...
  - Integrate with other components like `ManagerAgent` and `load_graph`.
- **Non-goals:**
...
## Evidence
- `neurosurfer/agents/graph/executor.py`: Core module for `GraphExecutor`.
- `neurosurfer/agents/graph/agent.py`: `GraphAgent` references `GraphExecutor`.
- `neurosurfer/agents/code/agent.py`: `CodeAgent` references `GraphExecutor`.
- `neurosurfer/agents/rag/agent.py`: `RAGAgent` references `GraphExecutor`.
- `neurosurfer/agents/react/agent.py`: `ReActAgent` references `GraphExecutor`.
- `neurosurfer/agents/graph/loader.py`: `load_graph` function is used for loading graphs.