...
description: A Neurosurfer app, explained step‑by‑step—app init, startup wiring, RAG, chat handler, shutdown, cooperative stop, and running the server.
---

# Example Application

This is a **guided walk‑through** of a Neurosurfer app. Instead of one giant snippet, we’ll build it piece by piece and explain how each section fits together. At the end, you’ll find the complete example in one block for easy copy‑paste.

> See also: [Lifecycle Hooks](./backend/lifecycle-hooks.md) • [Chat Handlers](./backend/chat-handlers.md) • [Configuration](../api-reference/configuration.md) • [Auth & Users](./backend/auth.md)
