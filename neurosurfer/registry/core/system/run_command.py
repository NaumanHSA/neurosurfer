...
from neurosurfer.tools.settings import WorkingDirectory
...
class RunCommandTool(Tool):
    capabilities = frozenset({"system.shell"})
    ...
    settings_model = WorkingDirectory
    root_setting = "root"
    # `cwd` is both an argument and a path, which is the point: the model may
    # still pick a subdirectory to run in, and the root is the boundary it picks
    # inside. Without this it could pick anywhere the gateway process can reach.
    path_inputs = frozenset({"cwd"})
