...
class WebSearchArgs(BaseModel):
    ...
    api_key: str | None = Field(
        default=None,
        description=(
            "SerpAPI key. Only needed for engine='serpapi'; falls back to the "
...
class WebSearchTool(Tool):
    ...
    # The key was reachable only as a process environment variable, so a hosted
    # gateway could offer SerpAPI to everyone or to no one, and a workflow could
    # not say which key it runs on. Declaring it lets the studio's credential
    # picker supply it and keeps it out of the graph, which is the same treatment
    # `sql` gives its `dsn`.
    secret_inputs = frozenset({"api_key"})
    credential_help = (
        "A SerpAPI key from serpapi.com — needed only for the 'serpapi' engine. "
        "DuckDuckGo needs no key at all."
    )
    ...
    async def call(self, args: WebSearchArgs, ctx: ToolContext) -> ToolResult:  # type: ignore[override]
        ...
        try:
            engine = get_engine(engine_name, api_key=args.api_key)
