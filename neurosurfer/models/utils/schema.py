...
class StructuredPromptOptions:
    ...
    header: str = "## Structured Output Contract"
...
def maybe_unwrap_named_root(candidate_json: str, schema_cls: Type[PydanticModel]) -> str:
    """
    ...
    try:
        ...
        return candidate_json  # not valid JSON yet; upstream repair may handle
...
def build_structured_system_prompt(
    ...
    *,
    ...
    shape = model_to_structure_block(
        schema_cls,
        title=opts.title,
...
def extract_first_json_object(text: str) -> Optional[str]:
    ...
    Accepts raw LLM output (with or without code fences).
    Returns the substring (no validation here) or None if not found.
    """
    code_fence_re = re.compile(r"^```.*?\n|```$", re.DOTALL)
    if not text:
        return None
    t = code_fence_re.sub("", text.strip())
