...
# Docs that name every feature in the project without explaining any of them.
# A roadmap entry is mostly a list of capability nouns, which is exactly the
# vocabulary a build's queries use — so BM25 scores it highly for almost anything
# and it displaces the guide that actually answers. Measured on six build-shaped
# queries it took a top-3 slot in four of them, once beating the graph guide for
# "tool node tool_args required arguments" with a section about trace spans.
_EXCLUDED_PREFIXES = ("about/roadmap", "about/changelog")


def _is_excluded(rel_path: str) -> bool:
    norm = rel_path.replace("\\", "/").lower()
    return norm.startswith(_EXCLUDED_PREFIXES)
