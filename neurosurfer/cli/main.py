...
def build_parser() -> argparse.ArgumentParser:
    ...
    s.add_argument("--ui-host", type=str, default=os.environ.get("NEUROSURFER_UI_HOST", config.app.ui_host))
    s.add_argument("--ui-port", type=int, default=int(os.environ.get("NEUROSURFER_UI_PORT", config.app.ui_port)))
    s.add_argument("--ui-open", default=os.environ.get("NEUROSURFER_UI_OPEN", True), help="Open browser for UI on start")
    s.add_argument("--ui-strict-port", action="store_true", help="Fail if UI port is already in use")

    s.add_argument("--npm-install", choices=["auto", "always", "never"], default=os.environ.get("NEUROSURFER_NPM_INSTALL", "auto"), help="First-run npm install behavior")
