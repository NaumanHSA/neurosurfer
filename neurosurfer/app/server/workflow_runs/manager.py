...
class MissingSecrets(RuntimeError):
    """A run was asked for that cannot connect, and we know why before starting.

    Distinct from a run *failure*: nothing was attempted, no record was created,
    and the fix is a setting rather than a change to the workflow.
    """
...
class RunManager:
    ...
    def __init__(
        ...
        *,
        ...
        # Carries the account's configured profiles so a node can name one.
        self.provider_resolver = provider_resolver
        # Called per run, not read once: a manager is cached for the account's
        # lifetime and a secret set five minutes from now must reach the next run
        # without anyone remembering to invalidate this object.
        self._secrets_provider = secrets_provider

    def _require_secrets(self, pkg: Any) -> None:
        """Refuse a run whose stored values are not set, naming them.

        Otherwise the first node fails with whatever the server says about an
        empty password — a connection error at the least informative moment,
        several steps from the setting that would fix it.
        """
        from neurosurfer.graph.workflow.requirements import missing_requirements
        from neurosurfer.mcp.credentials import use_credentials

        try:
            with use_credentials(self._secrets()):
                missing = missing_requirements(pkg)
        except Exception:  # noqa: BLE001 - a check that breaks must not block a run
            return
        if not missing:
            return
        detail = "; ".join(f"{r.name} (needed by {r.source})" for r in missing)
        raise MissingSecrets(
            f"This workflow needs values that are not set: {detail}. "
            f"Add them in Settings → Secrets and run it again."
        )

    def _secrets(self) -> dict[str, str]:
        """This account's stored environment values, fetched fresh per run."""
        if self._secrets_provider is None:
            return {}
        try:
            return dict(self._secrets_provider() or {})
        except Exception:  # noqa: BLE001 - a store fault must not stop a run
            return {}
    ...
    def start(
        ...
        def _work() -> None:
            try:
                def on_event(
                    node_id: str, status: str, scope: dict[str, Any] | None = None
                ) -> None:
                    if rec.cancelled:
                        return
                    if scope:
                        # A node inside a loop/map/subgraph body. It gets its own
                        # event (so a UI can animate the body live) but must not
                        # enter the top-level node map — body ids are scoped to
                        # their container and iterations would overwrite each other.
                        rec.add_event("node", node_id=node_id, status=status, scope=scope)
                        return
                ...
                for nid, nr in result.nodes.items():
                    ...
                    rec.set_node(
                        ...
                        # None for nodes that never called a model (function,
                        # tool, expression routers) — distinct from zero tokens.
                        usage=nr.usage.model_dump() if nr.usage else None,
                        tool_calls=list(nr.tool_calls),
                        input=nr.node_input,
                    )
                ...
                rec.usage = total.model_dump() if total.total() else None
                final = {k: _jsonable(v) for k, v in (result.final or {}).items()}
                if awaiting is not None:
                    rec.add_event("input_required", node_id=awaiting)
                    self.store.finish(rec, "awaiting_input", final=final,
                                      error=result.nodes[awaiting].error)
                elif result.errors:
                    self.store.finish(rec, "failed", final=final,
                                      error="; ".join(f"{k}: {v}" for k, v in result.errors.items()))
                else:
                    self.store.finish(rec, "succeeded", final=final)
                ...
                if rec.status not in TERMINAL:
                    self.store.finish(rec, "failed", error=str(e))
            finally:
                self.store.persist(rec)

        def _run_with_secrets() -> None:
            # Bound on this thread because that is where the runner (and any MCP
            # reconnect it triggers) will look; a raw thread inherits no context.
            from neurosurfer.mcp.credentials import use_credentials

            with use_credentials(self._secrets()):
                _work()

        threading.Thread(target=_run_with_secrets, name=f"wfrun-{rec.id[:8]}",
                         daemon=True).start()
        return rec
    ...
    def cancel(self, run_id: str) -> RunRecord | None:
        ...
        if rec.status not in TERMINAL:
            rec.request_cancel()
            self.store.finish(rec, "cancelled")
            self.store.persist(rec)
        return rec
