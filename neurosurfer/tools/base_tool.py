...
class BaseTool(ABC):
    ...
    def get_tool_description(self) -> str:
        """
        Generate a formatted description of the tool. This description is used by the agent to understand the tool.
        ...
        llm_req_inputs = [p for p in self.spec.inputs if p.llm]
        if llm_req_inputs:
            description.append("Tool Inputs:")
            for p in llm_req_inputs:
                req = "required" if p.required else "optional"
                description.append(f"- `{p.name}`: {p.type} ({req}) — {p.description}")
        else:
            description.append("Tool Inputs: None (The tool doesn't take any inputs)")
