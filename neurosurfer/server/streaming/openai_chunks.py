...
from ..schemas.openai import ChatCompletionChunk, ChatCompletionChunkChoice, ChatCompletionChunkDelta
...
def chunk_end(*, id: str, created: int, model: str, finish_reason: str = "stop") -> dict:
    return ChatCompletionChunk(
        ...
        created=created,
        model=model,
        choices=[ChatCompletionChunkChoice(index=0, delta=ChatCompletionChunkDelta(), finish_reason=finish_reason)]
    ).model_dump()
